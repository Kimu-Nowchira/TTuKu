...
import { ISession, IUser } from "../../../game/src/types"
...
import {
  ...
  session,
  users,
} from "../../../game/src/utils/db"
import { config } from "../../../game/src/config"
import { all, Tail } from "../../../game/src/utils/lizard"
...
export const run = (Server: Express, page) => {
  ...
  Server.get("/gwalli/gamsi", async (req, res) => {
    if (!checkAdmin(req, res)) return
    ...
    const data = {
      _id: $u._id,
      server: $u.server,
      title: $s.profile.title || $s.profile.name,
    }
...
function parseKKuTuHot() {
  ...
  all([
    ...
    query(
      `SELECT * FROM kkutu_ko WHERE type = 'INJEONG' AND hit > 0 ORDER BY hit DESC LIMIT 50`
    ),
