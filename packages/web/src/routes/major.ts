...
import { KO_INJEONG } from "../../../game/src/const"
import { kkutu, kkutu_injeong, kkutu_shop, redis, users } from "../../../game/src/utils/db"
...
function obtain($user, key, value, term, addValue = false) {
  ...
  if (term) {
    if ($user.box[key]) {
      ...
      $user.box[key] = { value: value, expire: Math.round(now * 0.001 + term) }
  } else {
    $user.box[key] = ($user.box[key] || 0) + value
  }
}

function consume($user, key, value, force = false) {
  var bd = $user.box[key]
...
export const run = (Server: Express, page) => {
    ...
    kkutu["ko"]
      ...
      .on(function ($word) {
        ...
        kkutu_injeong.findOne(["_id", word]).on(function ($ij) {
          ...
          Web.get(
            "https://namu.moe/w/" + encodeURI(word),
            function (err, _res) {
  ...
  function blendWord(word) {
    ...
    if (lang == "ko") {
      ...
      ;[0, 1, 2]
        .sort((a, b) => Number(Math.random() < 0.5))
        .forEach((v, i) => {
          kr.push(kl[v][i])
        })
