...
;(function () {
  ...
  var audioContext = window.hasOwnProperty("AudioContext")
    ? new AudioContext()
    : false
  ...
  $(document).ready(function () {
    ...
    if (mobile) TICK = 200
    $data._timePercent = false
      ? function () {
          return ($data._turnTime / $data.turnTime) * 100 + "%"
        }
      : function () {
          var pos = $data._turnSound.audio
    ...
    $data.setRoom = function (id, data) {
      ...
      if (data == null) {
        ...
        if (isLobby && !$data.rooms[id])
          $stage.lobby.roomList.append($("<div>").attr("id", "room-" + id))
        $data.rooms[id] = data
      ...
      if (data == null) {
        ...
        if (needed) {
          if ($obj) $("#" + $obj.attr("id")).replaceWith($obj)
            ...
            $(
              "#" + (only == "for-lobby" ? "users-item-" : "invite-item") + id
            ).replaceWith(userListBar(data, only == "for-master"))
        }
      }
    }
    ...
    for (i in $stage.dialog) {
      if ($stage.dialog[i].children(".dialog-head").hasClass("no-close"))
        continue
    ...
    $stage.dialog.quickOK.on("click", function (e) {
      ...
      function quickTick() {
        ...
        $("#quick-queue").html(
          L["quickQueue"] + " " + prettyTime($data._quickn++ * 1000)
        )
        for (i in $data.rooms) {
    ...
    $stage.dialog.dictSearch
      .on("click", function (e) {
        ...
        tryDict($("#dict-input").val(), function (res) {
          ...
          $("#dict-output").html(
            processWord(res.word, res.mean, res.theme, res.type.split(","))
          )
        })
      })
    ...
    $stage.dialog.profileWhisper.on("click", function (e) {
      ...
      $stage.talk
        .val(
          "/e " + (o.profile.title || o.profile.name).replace(/\s/g, "") + " "
        )
        .focus()
    })
  ...
  $lib.Classic.roundReady = function (data) {
    var i,
      len = $data.room.game.title.length
    var $l
    ...
    if ($data.room.opts.mission) {
      $stage.game.items
        .show()
        .css("opacity", 1)
        .html(($data.mission = data.mission))
    }
  ...
  $lib.Classic.turnStart = function (data) {
    ...
    $stage.game.display.html(
      ($data._char = getCharText(data.char, data.subChar, data.wordLength))
    )
  ...
  $lib.Classic.turnGoing = function () {
    ...
    if (!$stage.game.roundBar.hasClass("round-extreme"))
      if ($data._roundTime <= 5000)
    ...
    if (data.hint) {
      ...
      if (MODE[$data.room.mode] == "KAP")
        $stage.game.display
          .empty()
          .append(
            $("<label>").css("color", "#AAAAAA").html(data.hint.slice(0, hi))
          )
        ...
        $stage.game.display
          ...
          .append(
            $("<label>")
              .css("color", "#AAAAAA")
              .html(data.hint.slice(hi + 1))
          )
    }
    if (data.bonus) {
      mobile
        ? $sc.html("+" + (b.score - b.bonus) + "+" + b.bonus)
        : addTimeout(function () {
            var $bc = $("<div>")
  ...
  $lib.Jaqwi.turnGoing = function () {
    ...
    tt = $data._spectate
      ? L["stat_spectate"]
      : ($data._roundTime * 0.001).toFixed(1) + L["SECOND"]
  ...
  $lib.Jaqwi.turnEnd = function (id, data) {
    ...
    if (data.giveup) {
      ...
      $stage.game.here.hide()
      $stage.game.display.html(
        $("<label>").css("color", "#FFFF44").html(data.answer)
      )
      stopBGM()
      playSound("horr")
    } else {
  ...
  $lib.Crossword.turnEnd = function (id, data) {
    ...
    if (data.score) {
      ...
      if (id == $data.id) {
        ...
        if ($data._sel)
          if (data.pos.join(",") == $data._sel.join(","))
            $stage.game.cwcmd.css("opacity", 0)
        playSound("mission")
      }
      ...
      if (data.pos[0] == $data.selectedRound - 1) $lib.Crossword.drawDisplay()
        ...
        $cr = $($stage.game.round.children("label").get(data.pos[0])).addClass(
          "round-effect"
        )
        addTimeout(function () {
  ...
  $lib.Crossword.onBar = function (e) {
    ...
    $(".cw-q-body").html(
      processWord("★", data.mean, data.theme, data.type.split(","))
    )
  }
  ...
  function restGoing(rest) {
    $(".jjo-turn-time .graph-bar").html(rest + L["afterRun"])
  ...
  function drawSpeed(table) {
    ...
    for (i in table) {
      $("#game-user-" + i + " .game-user-score")
        .empty()
        .append(
          $("<div>")
            .css({ float: "none", color: "#4444FF", "text-align": "center" })
            .html(
              table[i] +
  ...
  $lib.Daneo.roundReady = function (data) {
    ...
    $data._roundTime = $data.room.time * 1000
    $stage.game.display.html(
      ($data._char = "&lt;" + L["theme_" + data.theme] + "&gt;")
    )
    $stage.game.chain.show().html(($data.chain = 0))
    if ($data.room.opts.mission) {
      $stage.game.items
  ...
  $lib.Sock.drawDisplay = function () {
    ...
    va.forEach(function (item, index) {
      $a.append(
        ($c = $("<div>")
          .addClass("sock-char sock-" + item)
          .css({ width: size, height: size })
          .html(item))
      )
      if ($data._va[index] && $data._va[index] != item) {
        $c.html($data._va[index])
          .addClass("sock-picked")
    ...
    while (true) {
      ...
      if (lv == 4) {
        if (
          ...
          return send(
            "caj",
            { answer: "yes", input: [input[1], input[2], input[0]] },
            true
          )
      } else {
        if (confirm(L["checkAgeCancel"]))
          return send("caj", { answer: "no" }, true)
      }
    }
  }
    ...
    switch (data.type) {
      ...
      case "connRoom":
        ...
        if ($target.id == $data.id) loading()
          ...
          notice(
            ($target.profile.title || $target.profile.name) + L["hasJoined"]
          )
        updateUserList()
        break
      case "disconnRoom":
      ...
      case "tail":
        notice(
          data.a +
            ...
            data.id +
            ": " +
            (data.msg instanceof String ? data.msg : JSON.stringify(data.msg))
              .replace(/</g, "&lt;")
              .replace(/>/g, "&gt;"),
          "tail"
        )
      ...
      case "friend":
        ...
        if ($data._friends[data.id] && $data.friends[data.id])
          notice(
            (data.stat == "on"
              ? "&lt;<b>" +
                L["server_" + $data._friends[data.id].server] +
                "</b>&gt; "
              : "") +
              L["friend"] +
              " " +
              $data.friends[data.id] +
              L["fstat_" + data.stat]
          )
        updateCommunity()
        break
      case "friendAdd":
        ...
        i =
          ($target.title || $target.name) + "(#" + data.from.substr(0, 5) + ")"
        send(
          "friendAddRes",
          {
            from: data.from,
      ...
      case "kickVote":
        ...
        notice(
          ($data._kickTarget.profile.title || $data._kickTarget.profile.name) +
            L["kickVoting"]
        )
        break
      case "kickDeny":
      ...
      case "inviteNo":
        ...
        notice(
          ($target.profile.title || $target.profile.name) + L["inviteDenied"]
        )
        break
      case "okg":
        if ($data._playTime > data.time) {
          ...
          notice(L["okgNotice"] + " (" + L["okgCurrent"] + data.count + ")")
      ...
      case "error":
        ...
        if (data.code == 401) {
          ...
          if (confirm(L["error_" + data.code])) {
            ...
            send(
              "enter",
              { id: data.target, password: $data._pw, spectate: true },
              true
            )
          }
          return
          ...
          if (i.indexOf("생년월일") != -1) {
            alert(
              "생년월일이 올바르게 입력되지 않아 게임 이용이 제한되었습니다. 잠시 후 다시 시도해 주세요."
            )
            break
          }
          ...
          var block =
            "\n제한 시점: " +
            blockedUntil.getFullYear() +
            "년 " +
            blockedUntil.getMonth() +
            1 +
            "월 " +
    ...
    switch (cmd[0].toLowerCase()) {
      ...
      case "/id":
        if (cmd[1]) {
          ...
          for (i in $data.users) {
            if (
              ($data.users[i].profile.title || $data.users[i].profile.name) ==
              cmd[1]
            ) {
              notice("[" + ++c + "] " + i)
            }
...
*/
    ...
    if (only == "for-lobby") {
      ...
      if (
        ...
        $data.users[$data.id].game.form == "S"
      ) {
      ...
      if (only == "for-master")
        if ($stage.dialog.inviteList.is(":visible")) updateUserList()
      updateRoom(false)
      updateMe()
  ...
  function checkRoom(modify) {
    ...
    var OBJ = {} + ""
    var i,
      arr = $data._players.split(",")
    var lb = arr.length,
      la = $data.room.players.length
    var u
  ...
  function updateMe() {
    ...
    $(".my-stat-level").replaceWith(
      getLevelImage(my.data.score).addClass("my-stat-level")
    )
  ...
  function userListBar(o, forInvite) {
    ...
    if (forInvite) {
      $R = $("<div>")
        .attr("id", "invite-item-" + o.id)
        .addClass("invite-item users-item")
        .append(
          $("<div>")
            .addClass("jt-image users-image")
            .css("background-image", "url('" + o.profile.image + "')")
        )
  ...
  function updateRoomList(refresh) {
    ...
    if (len) {
      ...
      $stage.lobby.roomList.append(
        $stage.lobby.createBanner.clone().on("click", onBanner)
      )
    }
    function onBanner(e) {
  ...
  function roomListBar(o) {
    ...
    $R = $("<div>")
      ...
      .append(
        ($ch = $("<div>")
          .addClass("rooms-channel channel-" + o.channel)
          .on("click", function (e) {
      ...
      .append(
        $("<div>").addClass("rooms-title ellipse").text(badWords(o.title))
      )
      .append(
        $("<div>")
          .addClass("rooms-limit")
          .html(o.players.length + " / " + o.limit)
      )
      .append(
        $("<div>")
          .width(270)
          .append(
            $("<div>").addClass("rooms-mode").html(opts.join(" / ").toString())
          )
          .append(
            $("<div>")
              .addClass("rooms-round")
              .html(L["rounds"] + " " + o.round)
          )
          .append(
            $("<div>")
              .addClass("rooms-time")
              .html(o.time + L["SECOND"])
          )
      )
      .append(
        $("<div>")
      ...
      .append(
        $("<div>")
          ...
          .append(
            $("<div>")
              .addClass("expl")
              .html(L["LEVEL"] + " " + getLevel(o.data.score))
          )
      )
    ...
    if (gaming) {
      ...
      for (i in $data.room.game.seq) {
        if ($data._replay) {
          ...
          o =
            $data.users[$data.room.game.seq[i]] ||
            $data.robots[$data.room.game.seq[i].id] ||
            $data.room.game.seq[i]
        }
      ...
      for (i in $data.room.players) {
          ...
          $("<div>")
            ...
            .append(
              $("<div>")
                ...
                .append(
                  ($z = $("<div>")
                    .addClass("room-user-team team-" + o.game.team)
                    .html($("#team-" + o.game.team).html()))
                )
            )
            .append(
              $("<div>")
        ...
        if (o.id == $data.room.master) {
          $y.addClass("room-user-master").html(
            L["master"] + prac + (spec || "")
          )
        } else if (spec) {
  ...
  function drawScore($obj, score) {
    var i,
      sc =
        score > 99999
          ? zeroPadding(Math.round(score * 0.001), 4) + "k"
          : zeroPadding(score, 5)
  ...
  function renderGoods($target, preId, filter, equip, onClick) {
    ...
    if (!equip) equip = {}
    for (i in equip) {
      if (!$data.box.hasOwnProperty(equip[i]))
        $data.box[equip[i]] = { value: 0 }
    }
    for (i in $data.box)
      list.push({ key: i, obj: iGoods(i), value: $data.box[i] })
    ...
    for (i in list) {
      ...
      equipped =
        g == "Mhand"
          ? equip["Mlhand"] == list[i].key || equip["Mrhand"] == list[i].key
          : equip[g] == list[i].key
    ...
    renderGoods($("#dress-goods"), "dress", filter, equip, function (e) {
      ...
      if (e.ctrlKey) {
        ...
        if (
          !confirm(
            L["surePayback"] +
              commify(Math.round((item.cost || 0) * 0.2)) +
              L["ping"]
          )
        )
          return
  ...
  function drawCharFactory() {
    ...
    function trayEmpty() {
      $tray.html(
        $("<h4>").css("padding-top", "8px").width("100%").html(L["cfTray"])
      )
    }
    function drawCFTray() {
        ...
        $("<tr>")
          ...
          .append(
            $("<td>")
              ...
              .append(
                $("<label>").css("padding-top", 2).html(getLevel(item.score))
              )
          )
    ...
    for (i in $data.friends) {
      ...
      $stage.dialog.commFriends.append(
        $("<div>")
          ...
          .append(
            $("<div>").addClass(
              "cfi-status cfi-stat-" + (o.server ? "on" : "off")
            )
          )
          .append(
            $("<div>")
              .addClass("cfi-server")
              .html(o.server ? L["server_" + o.server] : "-")
          )
          .append(
            $("<div>")
              .addClass("cfi-name ellipse")
              .html(p ? p.title || p.name : L["hidden"])
          )
          ...
          .append(
            $("<div>")
              .addClass("cfi-menu")
              .append(
                $("<i>").addClass("fa fa-pencil").on("click", requestEditMemo)
              )
              .append(
                $("<i>")
                  .addClass("fa fa-remove")
                  .on("click", requestRemoveFriend)
              )
          )
      )
    }
  ...
  function requestRoomInfo(id) {
    ...
    $("#ri-title").html(
      (o.password ? "<i class='fa fa-lock'></i>&nbsp;" : "") + o.title
    )
    ...
    o.players.forEach(function (p, i) {
        ...
        $("<div>")
          ...
          .append(
            ($p = $("<div>")
              ...
              .html(p.profile.title || p.profile.name))
          )
          .append(
            $("<div>")
              .addClass("rip-team team-" + rd.t)
              .html($("#team-" + rd.t).html())
          )
          .append(
            $("<div>")
      ...
      $p.prepend(
        getLevelImage(p.data.score).addClass("profile-level rip-level")
      )
  ...
  function requestProfile(id) {
    ...
    $("#ProfileDiag .dialog-title").html(
      (o.profile.title || o.profile.name) + L["sProfile"]
    )
    $(".profile-head")
      .empty()
      .append(($pi = $("<div>").addClass("moremi profile-moremi")))
      .append(
        $("<div>")
        ...
        $("<div>")
          ...
          .append(
            $("<div>")
              ...
              .html(L["LEVEL"] + " " + (i = getLevel(o.data.score)))
          )
          .append(
            $("<div>")
              .addClass("profile-score-text")
              .html(
                commify(o.data.score) + " / " + commify(EXP[i - 1]) + L["PTS"]
              )
          )
      )
      .append(
        ($ex = $("<div>")
          ...
          .append(
            $("<div>")
              .addClass("expl")
              .css({ "white-space": "normal", width: 300, "font-size": "11px" })
              .text(o.exordial)
          ))
      )
    if (o.robot) {
          ...
          $("<div>")
            ...
            .append(
              $("<div>")
                ...
                .html(r[0] + L["P"] + " " + r[1] + L["W"])
            )
            .append(
              $("<div>")
                .addClass("profile-field-score")
                .html(commify(r[2]) + L["PTS"])
            )
        )
      }
  ...
  function gameReady() {
    ...
    for (i in $data.room.players) {
      if ($data._replay) {
        ...
        u =
          $data.users[$data.room.players[i]] ||
          $data.robots[$data.room.players[i].id]
      }
      u.game.score = 0
  ...
  function replayStatus() {
    $data.$gpt.html(
      $data._gpp +
        " (" +
        ($data._eventTime * 0.001).toFixed(1) +
        L["SECOND"] +
        " / " +
        ($data._gtt * 0.001).toFixed(1) +
        L["SECOND"] +
        ")"
    )
  }
  ...
  function recordEvent(data) {
    ...
    if (data.profile)
      data.profile = { id: data.profile.id, title: "#" + data.profile.id }
    if (data.user)
      data.user = {
        id: data.user.profile.id,
        profile: {
          id: data.user.profile.id,
          title: "#" + data.user.profile.id,
        },
        data: { score: 0 },
        equip: {},
      }
  ...
  function clearBoard() {
    ...
    $(".jjo-turn-time .graph-bar")
      .width(0)
      .css({ float: "", "text-align": "", "background-color": "" })
  ...
  function drawRound(round) {
    ...
    for (i = 0; i < $data.room.round; i++) {
      $stage.game.round.append(
        ($l = $("<label>").html($data.room.game.title[i]))
      )
  ...
  function turnError(code, text) {
    $stage.game.display.empty().append(
      $("<label>")
        ...
        .text(
          (L["turnError_" + code] ? L["turnError_" + code] + ": " : "") + text
        )
    )
    playSound("fail")
    clearTimeout($data._fail)
  ...
  function roundEnd(result, data) {
    ...
    for (i in result) {
      ...
      lvUp =
        getLevel((sc = o.data.score)) > getLevel(o.data.score - r.reward.score)
            ...
            $("<div>")
              ...
              .html(
                data.scores
                  ...
                  : commify(r.score || 0) + L["PTS"]
              )
          )
          .append(
            $("<div>")
              .addClass("result-board-reward")
              .html(r.reward.score ? "+" + commify(r.reward.score) : "-")
          )
          .append(
            $("<div>")
              .addClass("result-board-lvup")
              .css("display", lvUp ? "block" : "none")
      ...
      if (r.id == $data.id) {
        ...
        $(".result-me-expl").append(
          explainReward(r.reward._score, r.reward._money, r.reward._blog)
        )
      }
    }
    ...
    if ($data._result) {
      ...
      if (addit > 0) {
        addit =
          "<label class='result-me-bonus'>(+" + commify(addit) + ")</label>"
      } else addit = ""
      if (addp > 0) {
      ...
      $(".result-me-score").html(
        L["scoreGain"] + " +" + commify($data._result.reward.score) + addit
      )
      $(".result-me-money").html(
        L["moneyGain"] + " +" + commify($data._result.reward.money) + addp
      )
    }
    function roundEndAnimation(first) {
      ...
      draw(
        "current",
        Math.min(going, $data._result._boing),
        0,
        $data._result.goal - $data._result.before
      )
      draw(
        "bonus",
        Math.max(0, going - $data._result._boing),
        0,
        $data._result.goal - $data._result.before
      )
    ...
    function draw(phase, val, before, goal) {
      $(".result-me-" + phase + "-bar").width(
        ((val - before) / (goal - before)) * 100 + "%"
      )
    }
    ...
    var $R = $("<div>")
      ...
      .append(
        $("<div>")
          ...
          .html(iName(item._id) + (equipped ? L["equipped"] : ""))
      )
      .append(
        $("<div>")
          .addClass("dress-item-group")
          .html(L["GROUP_" + item.group])
      )
    ...
    if (item.term)
      $R.append(
        $("<div>")
          ...
          .html(
            Math.floor(item.term / 86400) + L["DATE"] + " " + L["ITEM_TERM"]
          )
      )
    if (expire)
      $R.append(
        $("<div>")
          .addClass("dress-item-term")
          .html(new Date(expire * 1000).toLocaleString() + L["ITEM_TERMED"])
      )
  ...
  function onGoods(e) {
    ...
    ceq[
      $obj.group == "Mhand"
        ? ["Mlhand", "Mrhand"][Math.floor(Math.random() * 2)]
        : $obj.group
    ] = id
  ...
  function pushDisplay(text, mean, theme, wc) {
    ...
    if (beat) {
      ...
      $(i).css(
        isRev ? "margin-right" : "margin-left",
        ($stage.game.display.width() - 20 * len) * 0.5
      )
    } else {
      j = ""
      if (isRev)
    ...
    $stage.game.hints.append(
      ($obj = $("<div>")
        ...
        .append(
          $("<div>")
            .addClass("expl")
            .css({ "white-space": "normal", width: 200 })
            .html(v.html())
        ))
    )
  ...
  function pushHistory(text, mean, theme, wc) {
    ...
    $v.append(
      ($w = $("<div>").addClass("history-mean ellipse").append(val))
    ).append(
      ($x = $("<div>")
        .addClass("expl")
        .css({ width: 200, "white-space": "normal" })
  ...
  function processWord(word, _mean, _theme, _wcs) {
    ...
    var types = _wcs
      ? _wcs.map(function (_wc) {
          return L["class_" + _wc]
        })
      : []
    var themes = _theme
      ? _theme.split(",").map(function (_t) {
          return L["theme_" + _t]
        })
      : []
  ...
  function getCharText(char, subChar, wordLength) {
    ...
    if (wordLength)
      res +=
        "<label class='jjo-display-word-length'>(" + wordLength + ")</label>"
  ...
  function getRequiredScore(lv) {
    return Math.round(
      (!(lv % 5) * 0.3 + 1) *
        ...
        (120 +
          Math.floor(lv / 5) * 60 +
          Math.floor((lv * lv) / 225) * 120 +
          Math.floor((lv * lv) / 2025) * 180)
    )
  }
    ...
    $obj
      ...
      .append(
        $("<h5>")
          ...
          .html("[" + (room.practice ? L["practice"] : room.id) + "]")
      )
      .append($("<h5>").addClass("room-head-title").text(badWords(room.title)))
      .append(
        ($rm = $("<h5>").addClass("room-head-mode").html(opts.join(" / ")))
      )
      .append(
        $("<h5>")
      ...
      .append(
        $("<h5>")
          ...
          .html(L["rounds"] + " " + room.round)
      )
      .append(
        $("<h5>")
          .addClass("room-head-time")
          .html(room.time + L["SECOND"])
      )
    ...
    if (rule.opts.indexOf("ijp") != -1) {
      $rm.append(
        $("<div>")
          ...
          .html(
            "<h5>" +
              room.opts.injpick.map(function (item) {
  ...
  function chatBalloon(text, id, flag) {
    $("#cb-" + id).remove()
    var offset = (
      flag & 2 ? $("#game-user-" + id) : $("#room-user-" + id)
    ).offset()
  ...
  function chat(profile, msg, from, timestamp) {
    ...
    if (!mobile && $data.room) {
      $bar =
        ($data.room.gaming ? 2 : 0) + ($(".jjoriping").hasClass("cw") ? 1 : 0)
    ...
    $stage.chat.append(
      ($item = $("<div>")
        ...
        .append(
          ($bar = $("<div>")
            .addClass("chat-head ellipse")
            .text(profile.title || profile.name))
        )
        .append(($msg = $("<div>").addClass("chat-body").text(msg)))
        .append(
          $("<div>").addClass("chat-stamp").text(time.toLocaleTimeString())
        ))
    )
    ...
    if ((link = msg.match(/https?:\/\/[\w\.\?\/&#%=-_\+]+/g))) {
      ...
      link.forEach(function (item) {
        msg = msg.replace(
          item,
          "<a href='#' style='color: #2222FF;' onclick='if(confirm(\"" +
            L["linkWarning"] +
            '")) window.open("' +
            item +
            "\");'>" +
  ...
  function iImage(key, sObj) {
    ...
    if (key) {
      ...
      sObj = { _id: "def", group: sObj, options: {} }
    obj = $data.shop[key] || sObj
    gif = obj.options.hasOwnProperty("gif") ? ".gif" : ".png"
    if (obj.group.slice(0, 3) == "BDG")
      return "/img/kkutu/moremi/badge/" + obj._id + gif
    ...
    $obj.children(".moremi-back").after(
      $("<img>")
        ...
        .attr(
          "src",
          equip.robot
            ? "/img/kkutu/moremi/robot.png"
            : "/img/kkutu/moremi/body.png"
        )
        .css({ width: "100%", height: "100%" })
    )
