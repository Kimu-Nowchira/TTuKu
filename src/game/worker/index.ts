...
import { DEVELOP, GUEST_PERMISSION } from "../master"
...
export const run = async () => {
  ...
  const { socket, info } = await new Promise<{
    ...
    Server.on("error", (err) => {
      logger.error("Error on ws: " + err.toString())
      rej(err)
    })
  })
  ...
  // IP 차단 여부 확인
  if (!info.connection.remoteAddress)
    throw new Error("No remoteAddress on IncomingMessage")
